...
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, 0xFF0000AA, "| ERRO | Comando Inv�lido.");
	IniciarObras();
	IniciouObras = true;
...
{
	if(IniciouObras == false) return SendClientMessage(playerid, 0xFF0000AA, "| OBRAS | N�o h� obras em Andamento!!");
    if(!IsPlayerInRangeOfPoint(playerid, 2.0, 1809.5896,822.3916,10.6904) && !IsPlayerInRangeOfPoint(playerid, 2.0, 1786.3190,822.0200,10.6953) && !IsPlayerInRangeOfPoint(playerid, 2.0, 1701.5922,381.8859,30.0385) && !IsPlayerInRangeOfPoint(playerid, 2.0, 1683.8262,388.5136,30.1807))return SendClientMessage(playerid, 0xFF0000AA, "| OBRAS | Voc� n�o est� proximo a um oper�rio.");
	new String[2000];
    format(String, sizeof(String), "{FFFFFF}Ol� {00FF00}%s{FF00FF}\n\n{FFFFFF}Este local est� fechado para a constru��o de um ped�gio.\n\nA previs�o para a conclus�o das obras � dia {FF0000}20/02/2024{FFFFFF}.\n\n\n{FF0000}AVISO{FFFFFF}: N�O ENTRE NA AREA DE CONSTRU��O, � PERIGOSO!\n\n\n\n{FFFFFF}Obrigado pela compreens�o, tenha um bom jogo!", PlayerName(playerid));
   	ShowPlayerDialog(playerid, 3500, DIALOG_STYLE_MSGBOX, "{FFFF00}Informa��es da Obra", String, "Ok", "");
	return 1;
}
